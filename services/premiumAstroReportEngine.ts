...
import { calculateBirthProfile } from '../src/services/astrology-calculator.ts';
import { getLogicalNekathForMonth } from '../src/data/nekathData.ts';
import {
  AstroDeterministicData,
  AstroFullReportJson,
  AstroReportInputSnapshot,
  AstroReportOrder,
  AstroReportOrderStatus,
  AstroReportRecord,
...
type AttachStripeCheckoutInput = {
  orderId: string;
  reportId: string;
  checkoutUrl: string;
  sessionId: string;
...
type SubmitInputsPayload = {
  ...
  palmQuality: AstroReportInputSnapshot['palmQuality'];
};

const PRODUCT_TYPE = 'full_astro_report' as const;
...
const sinhalaDate = (iso: string) => new Date(iso).toLocaleString('si-LK', { dateStyle: 'medium', timeStyle: 'short' });

const createId = (prefix: string) =>
  `${prefix}_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;

const hashCode = (input: string) =>
  Array.from(input).reduce((acc, char) => ((acc << 5) - acc + char.charCodeAt(0)) | 0, 0);

const seeded = (seed: number, offset = 0) => {
...
export class PremiumAstroReportEngine {
  ...
  createOrder({ userId, profile }: CreateOrderInput) {
    ...
    if (reusable) {
      ...
      return {
        ...
        payment: {
          integrationMode: PAYMENT_GATEWAY === 'stripe' ? ('stripe' as const) : ('placeholder' as const),
          checkoutToken: createId('checkout'),
          supportedStates: ['pending', 'paid', 'failed', 'cancelled', 'refunded'] as AstroReportOrderStatus[],
          checkoutUrl: null,
          sessionId: null,
          stripePriceId: null,
          displayAmount: null,
    ...
    const report: AstroReportRecord = {
      id: createId('report'),
      userId,
      orderId: order.id,
  ...
  attachStripeCheckout(input: AttachStripeCheckoutInput) {
    ...
    return {
      ...
      report,
      payment: {
        integrationMode: 'stripe' as const,
        checkoutToken: createId('checkout'),
        supportedStates: ['pending', 'paid', 'failed', 'cancelled', 'refunded'] as AstroReportOrderStatus[],
  ...
  confirmPayment(orderId: string, outcome: 'paid' | 'failed' | 'cancelled') {
    ...
    return {
      ...
      report,
      payment: {
        integrationMode: order.paymentGateway === 'stripe' ? ('stripe' as const) : ('placeholder' as const),
        checkoutToken: createId('checkout'),
        supportedStates: ['pending', 'paid', 'failed', 'cancelled', 'refunded'] as AstroReportOrderStatus[],
        checkoutUrl: null,
        sessionId: null,
  ...
  confirmStripePayment(orderId: string, paymentReference: string) {
    const orders = this.getOrders();
    const reports = this.getReports();
    const order = orders.find((entry) => entry.id === orderId);
    const report = reports.find((entry) => entry.orderId === orderId);
  ...
  getRequirements(reportId: string, userId: string, profile: UserProfile | null): AstroReportRequirements {
  ...
  submitInputs(reportId: string, payload: SubmitInputsPayload) {
    ...
    const inputSnapshot: AstroReportInputSnapshot = {
  ...
  createBackgroundReportFromProfile(userId: string, profile: UserProfile | null) {
    const activeStatuses: AstroReportStatus[] = ['collecting_inputs', 'queued', 'generating', 'pdf_generating'];
  ...
  listReports(userId: string) {
    ...
    const activeStatuses: AstroReportStatus[] = [
      'collecting_inputs',
      'queued',
      'generating',
      'pdf_generating',
  ...
  retryReport(reportId: string, adminKey: string) {
  ...
  getPalmImageFile(reportId: string, userId: string) {
    const report = this.requireReport(reportId, userId);
  ...
  private getOrders() {
    return readJsonFile<AstroReportOrder[]>(this.ordersFile, []);
  }

  private findReusablePendingBundle(userId: string) {
    const orders = this.getOrders()
  ...
  private saveOrders(orders: AstroReportOrder[]) {
    writeJsonFile(this.ordersFile, orders);
  }

  private upsertOrder(order: AstroReportOrder) {
    const orders = this.getOrders();
  ...
  private getReports() {
    return readJsonFile<AstroReportRecord[]>(this.reportsFile, []);
  }

  private saveReports(reports: AstroReportRecord[]) {
    writeJsonFile(this.reportsFile, reports);
  }

  private upsertReport(report: AstroReportRecord) {
    const reports = this.getReports();
  ...
  private requireReport(reportId: string, userId?: string) {
  ...
  private requireOrder(orderId: string, userId?: string) {
  ...
  private prefillFromProfile(profile: UserProfile | null): Partial<AstroReportInputSnapshot> {
    return {
  ...
  private validateInputPayload(payload: SubmitInputsPayload) {
    const errors: string[] = [];
  ...
  private storePalmImage(reportId: string, base64: string, mimeType: string) {
  ...
  private queueReportGeneration(reportId: string) {
  ...
  private async buildFinalReport(report: AstroReportRecord, deterministicData: AstroDeterministicData): Promise<AstroFullReportJson> {
    ...
    const sectionMap = sections.reduce<Record<string, AstroReportSection>>((acc, section) => {
      acc[section.key] = {
        key: section.key,
        title: section.title,
  ...
  private async generateSectionWithRetry(
    key: string,
    title: string,
    input: AstroReportInputSnapshot,
    deterministicData: AstroDeterministicData
  ) {
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      try {
        ...
        const parsed = safeJsonParse<{ content?: string }>(response.text || '', { content: lastContent });
