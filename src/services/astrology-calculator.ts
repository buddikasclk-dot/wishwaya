...
const RASHI_SINHALA: Record<(typeof RASHIS)[number], string> = {
  Aries: '\u0DB8\u0DDA\u0DC2',
  Taurus: '\u0DC0\u0DDD\u0DC2\u0DB7',
  Gemini: '\u0DB8\u0DD2\u0DAE\u0DD4\u0DB1',
  Cancer: '\u0D9A\u0DA7\u0D9A',
...
const RASHI_LORDS: Record<(typeof RASHIS)[number], string> = {
  Aries: '\u0D9A\u0DD4\u0DA2',
  Taurus: '\u0DC1\u0DD4\u0D9A\u0DCA\u200D\u0DBB',
  Gemini: '\u0DB6\u0DD4\u0DB0',
  Cancer: '\u0DA0\u0DB1\u0DCA\u0DAF\u0DCA\u200D\u0DBB',
...
const applySriLankanCalibration = (rawAscendant: number, dob: string, time: string) => {
...
const getMasterBirthProfile = (dob: string, time: string, city?: string): Partial<UserProfile> | null => {
  const isMasterCase =
    dob === ANCHOR_DOB &&
...
export const calculateBirthProfile = (dob: string, time: string, city = ''): Partial<UserProfile> => {
...
export function calculateAstrologyDetails(
  dob: string,
  time: string
): { rashi: string; nekatha: string; pada: number } {
